...
from ppocr.modeling.efficientnet.utils import MemoryEfficientSwish, Swish
from ppocr.modeling.efficientnet.utils_extra import Conv2dStaticSamePadding, MaxPool2dStaticSamePadding
import paddle.nn.functional as F
...
#             nn.BatchNorm2D(num_features=num_channels, momentum=0.9997, epsilon=4e-5), nn.ReLU())
#
...
class SeparableConvBlock(nn.Layer):
    ...
    def __init__(self, in_channels, out_channels=None, norm=True, activation=False, onnx_export=False):
        ...
        self.depthwise_conv = Conv2dStaticSamePadding(in_channels, in_channels,
                                                      kernel_size=3, stride=1, groups=in_channels, bias_attr=False)
...
class BiFPN(nn.Layer):
    ...
    def __init__(self, num_channels, conv_channels, first_time=False, epsilon=1e-4, onnx_export=False, attention=True,
        ...
        if self.first_time:
            self.p5_down_channel = nn.Sequential(
                ...
                nn.BatchNorm2D(num_channels, momentum=0.99, epsilon=1e-3),
            )
        ...
        # Weight
        self.p6_w1 = paddle.create_parameter(shape=[2], dtype='float32', default_initializer=paddle.nn.initializer.Constant(value=1.0))
        self.p6_w1_relu = nn.ReLU()
        self.p5_w1 = paddle.create_parameter(shape=[2], dtype='float32', default_initializer=paddle.nn.initializer.Constant(value=1.0))
        ...
        self.p5_w2 = paddle.create_parameter(shape=[3], dtype='float32', default_initializer=paddle.nn.initializer.Constant(value=1.0))
        self.p5_w2_relu = nn.ReLU()
        self.p6_w2 = paddle.create_parameter(shape=[3], dtype='float32', default_initializer=paddle.nn.initializer.Constant(value=1.0))
        self.p6_w2_relu = nn.ReLU()
        self.p7_w2 = paddle.create_parameter(shape=[2], dtype='float32', default_initializer=paddle.nn.initializer.Constant(value=1.0))
    ...
    def _forward_fast_attention(self, inputs):
        ...
        weight = p6_w1 / (paddle.sum(p6_w1, axis=0) + self.epsilon)
        # Connections for P6_0 and P7_0 to P6_1 respectively
        p6_up = self.conv6_up(self.swish(weight[0] * p6_in + weight[1] * self.p6_upsample(p7_in)))

        # Weights for P5_0 and P6_1 to P5_1
        p5_w1 = self.p5_w1_relu(self.p5_w1)
        weight = p5_w1 / (paddle.sum(p5_w1, axis=0) + self.epsilon)
        ...
        weight = p6_w2 / (paddle.sum(p6_w2, axis=0) + self.epsilon)
        ...
        weight = p7_w2 / (paddle.sum(p7_w2, axis=0) + self.epsilon)
    ...
    def _forward(self, inputs):
        ...
        if self.use_p8:
            ...
            return p3_out, p4_out, p5_out, p6_out, p7_out
    # def __init__(self, num_channels, out_channels, epsilon=1e-4):
    #     super(BiFPN, self).__init__()
    #     self.epsilon = epsilon
    #     # Conv layers
    #     self.conv6_up = ConvBlock(num_channels[0])
    #     self.conv5_up = ConvBlock(num_channels[1])
    #     self.conv4_up = ConvBlock(num_channels[2])
    #     self.conv3_up = ConvBlock(num_channels[3])
    ...
    #
    #     # Weight
    #     self.p6_w1 = paddle.create_parameter(paddle.ones([2, 2], dtype="float32"))
    #     self.p6_w1_relu = nn.ReLU()
    #     self.p5_w1 = paddle.create_parameter(paddle.ones([2, 2], dtype="float32"))
    #     self.p5_w1_relu = nn.ReLU()
    #     self.p4_w1 = paddle.create_parameter(paddle.ones([2, 2], dtype="float32"))
    #     self.p4_w1_relu = nn.ReLU()
    ...
    #     # self.p4_w2_relu = nn.ReLU()
    #     self.p5_w2 = paddle.create_parameter(paddle.ones([3, 3], dtype="float32"))
    #     self.p5_w2_relu = nn.ReLU()
    #     self.p6_w2 = paddle.create_parameter(paddle.ones([3, 3], dtype="float32"))
    #     self.p6_w2_relu = nn.ReLU()
    #     self.p7_w2 = paddle.create_parameter(paddle.ones([3, 3], dtype="float32"))
    #     self.p7_w2_relu = nn.ReLU()
    #
    # def forward(self, inputs):
    #     """
