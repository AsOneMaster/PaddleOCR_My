...
class BiFPN(nn.Layer):
    ...
    def forward(self, inputs):
        ...
        p4_up = p4_in + F.upsample(p5_in, scale_factor=2, mode="nearest", align_mode=1)
        print(p4_up.shape)
        # Weights for P5_0 and P6_0 to P5_1
        # p3_w1 = self.p3_w1_relu(self.p3_w1)
        # weight = p3_w1 / (paddle.sum(p3_w1,  axis=0) + self.epsilon)
        # Connections for P5_0 and P6_0 to P5_1 respectively
        p3_up = p3_in + F.upsample(p4_up, scale_factor=2, mode="nearest", align_mode=1)

        # Weights for P4_0, P4_1 and P3_2 to P4_2
        # p2_w1 = self.p2_w1_relu(self.p2_w1)
        # weight = p2_w1 / (paddle.sum(p2_w1,  axis=0) + self.epsilon)
        # Connections for P4_0, P4_1 and P3_2 to P4_2 respectively
        p2_out = p2_in + F.upsample(p3_up, scale_factor=2, mode="nearest", align_mode=1)
